class Game {
	...
	init() {
		...
		this.scene.background = new THREE.Color( 0x1af1f4 );
		...
		// ground
		var mesh = new THREE.Mesh( new THREE.PlaneBufferGeometry( 10000, 10000 ), new THREE.MeshPhongMaterial( { color: 0x4a6046, depthWrite: false } ) );
		...
		//player
		loader.load( `${this.assetsPath}fbx/people/Nathan.fbx`, function ( object ) {
            ...
            tLoader.load(`${game.assetsPath}images/Nathan.png`, function(texture){
				object.traverse( function ( child ) {
					if ( child.isMesh ){
    ...
    createColliders(){
        ...
        for (let x=10; x<1010; x+=250){
            for (let z=500; z<1500; z+=250){
